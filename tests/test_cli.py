#!/usr/bin/env python3
"""
Self-tests for the mozhi-pkg CLI.
...
# Copy the mozhi script to a .py file so we can import it as a module
import shutil as _shutil
_mozhi_py = PROJECT_ROOT / "tests" / "_mozhi_import.py"
_shutil.copy2(PROJECT_ROOT / "mozhi", _mozhi_py)
import _mozhi_import as mozhi_module

MOZHI = str(PROJECT_ROOT / "mozhi")
if not Path(MOZHI).exists():
    MOZHI = shutil.which("mozhi") or "mozhi"


def run_mozhi(*args, cwd=None):
    """Run the mozhi CLI and return (returncode, stdout, stderr)."""
    result = subprocess.run(
        [sys.executable, MOZHI] + list(args),
...
class TestSemVer(unittest.TestCase):
    ...
    def test_parse_simple(self):
        from _mozhi_import import SemVer
    ...
    def test_version_req_caret(self):
        from _mozhi_import import SemVer, parse_version_req
...
class TestTOMLParser(unittest.TestCase):
    ...
    def test_basic_string(self):
        from _mozhi_import import parse_toml
...
class TestCLICommands(unittest.TestCase):
    ...
    def test_version(self):
        rc, stdout, _ = run_mozhi("version")
        self.assertEqual(rc, 0)
        self.assertIn("mozhi-pkg", stdout)
        self.assertIn("1.2.0", stdout)

    def test_help(self):
        rc, stdout, _ = run_mozhi("help")
        self.assertEqual(rc, 0)
        self.assertIn("mozhi", stdout.lower())

    def test_init_lib(self):
        os.chdir(self.tmpdir)
        rc, _, stderr = run_mozhi("init", "--lib", "testlib")
        self.assertEqual(rc, 0)
        self.assertTrue(Path(self.tmpdir / "testlib" / "mozhi.toml").exists() if hasattr(self.tmpdir, '__truediv__') else (Path(self.tmpdir) / "testlib" / "mozhi.toml").exists())
        # Check structure
        lib_dir = Path(self.tmpdir) / "testlib"
        self.assertTrue((lib_dir / "mozhi.toml").exists())
    ...
    def test_init_bin(self):
        os.chdir(self.tmpdir)
        rc, _, _ = run_mozhi("init", "--bin", "testapp")
        self.assertEqual(rc, 0)
        app_dir = Path(self.tmpdir) / "testapp"
        self.assertTrue((app_dir / "mozhi.toml").exists())
        self.assertTrue((app_dir / "src" / "main.mz").exists())

    def test_build_native(self):
        os.chdir(self.tmpdir)
        run_mozhi("init", "--lib", "buildtest")
        os.chdir(Path(self.tmpdir) / "buildtest")
        rc, _, stderr = run_mozhi("build", "--native")
    ...
    def test_add_remove(self):
        os.chdir(self.tmpdir)
        run_mozhi("init", "--lib", "deptest")
        os.chdir(Path(self.tmpdir) / "deptest")
        rc, _, _ = run_mozhi("add", "github:testuser/mydep", "1.0.0")
        self.assertEqual(rc, 0)
        # Check it was added to mozhi.toml
        toml = (Path(self.tmpdir) / "deptest" / "mozhi.toml").read_text()
        self.assertIn("github:testuser/mydep", toml)
        # Remove it
        rc, _, _ = run_mozhi("remove", "github:testuser/mydep")
        self.assertEqual(rc, 0)
        toml = (Path(self.tmpdir) / "deptest" / "mozhi.toml").read_text()
